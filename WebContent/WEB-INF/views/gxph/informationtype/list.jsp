...
<body>
			...
			<form class="form-inline" role="form">
				<div class="form-group">
						<div class="input-group">
						   <div class="input-group-addon">分区分类名称</div>
						   <input class="form-control" name="search_LIKE_name" type="text"
							  placeholder="输入分区分类名称" value="${param.search_LIKE_name}">
					   </div>
				</div>
				...
				<div class="pull-right">
					<div class="btn-group">
						<a href="${ctx}/infortype/create" role="button"
							class="btn btn-primary">新增分区</a>
					</div>
				</div>
			</form>
		</div>
			...
			<table
				...
				<tbody>
					<c:forEach items="${infor.content}" var="info">
						<tr>
							<td><a href="${ctx}/infortype/update/${info.id}">编辑</a>
							<a href="javascript:void(0)" onclick="openDeleteModal('${info.id}')">删除</a></td>
							<td>
							<a href="${ctx}/infortype/senclist/${info.id}">${info.name}</a>
							</td>
							<td>${info.seq}</td>
							<td><c:if test="${info.isbelongtype == '0'}">不属于</c:if>
							<c:if test="${info.isbelongtype == '1'}">属于</c:if></td>
	...
	<script type="text/javascript">
		...
		function deleteSubject() {
			...
			window.location.href = '${ctx}/infortype/delete/'
				+ $('#deletecode').text();
		}
		function rest(){
			window.location.href = '${ctx}/infortype/';
		}
	</script>
</body>
</html>