...
<head>
<title><c:choose>
		<c:when test="${action=='create'}">新增主题分类</c:when>
		<c:when test="${action=='update'}">编辑主题分类</c:when>
...
<body>
	<div class="panel panel-default">
		<ol class="breadcrumb">
			<li><a href="">首页管理</a></li>
			<li><a href="${ctx}/subject">主题分类</a></li>
		...
		<div class="panel-body">
			<form id="inputForm" class="form-horizontal" role="form"
				action="${ctx}/subject/${action}" method="post" enctype="multipart/form-data">
				<legend>主题分类</legend>
				...
				<div class="form-group">
					...
					<div class="col-sm-8">
						<input type="text" class="form-control required" name="name"
							value="${subject.name}" placeholder="请输入主题名称" 
							/>
					</div>
				</div>
				<div class="form-group">
					...
					<div class="col-sm-8">
						<input type="text" class="form-control required" id="seq" name="seq" min="1" maxlength="10"
							 placeholder="请输入排序" value="${subject.seq}"
						/>
					</div>
				</div>
				<div class="form-group">
					...
					<div class="col-sm-8">
						<select class="form-control required" name="state">
							<option
								<c:if test="${subject.state == '1'}"> selected="selected"</c:if>
								value="1">正常</option>
							<option
								<c:if test="${subject.state == '0'}"> selected="selected"</c:if>
								value="0">停用</option>
						</select>
					</div>
				</div>
	...
	<script>
		...
		//为inputForm注册validate函数
			if (action == "create") {
				...
				$("#imgPreview").show();
				var subject = '${subject.apFileEnclosure}';
				if(subject){
					var pathRel = '${subject.apFileEnclosure.pathRel}';
