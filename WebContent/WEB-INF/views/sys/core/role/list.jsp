			...
			<table
				...
				<tbody>
					<c:forEach items="${list.content}" var="role">
						<tr>
							<td>
								<a href="${ctx}/coreRole/update/${role.id}">编辑</a>
								<a href="javascript:void(0)" onclick="openDeleteModal('${role.id}')">删除</a>
							</td>
							<td>${role.name}</td>
							<td>${role.des}</td>
							<td>
								<a class="btn btn-primary" href="${ctx}/coreRoleResource/role/${role.id}">查看/分配权限</a>
							</td>
						</tr>
					</c:forEach>	
				</tbody>
			</table>
			<c:if test="${list.content.size()==0||list.content==null}">
