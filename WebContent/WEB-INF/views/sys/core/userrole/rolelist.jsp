...
<body>
				...
				<div class="form-group">
						<div class="input-group">
						   ...
						   <input class="form-control" name="search_LIKE_name" type="text"
							  placeholder="请输入角色名称" value="${param.search_LIKE_name}">
					   </div>
				</div>
		...
		<div >
			<form action="${ctx}/coreUserRole/save" method="post">
			<table
				class="table table-bordered table-hover table-striped sortable">
				<thead>
					<tr>
						<th style="text-align: inherit;" data-defaultsort='disabled' class="col-md-1 col-xs-1">操作</th>
						<th style="text-align: inherit;">角色名称</th>
						<th style="text-align: inherit;">备注信息</th>
					</tr>
				</thead>
				<tbody>
					<c:forEach items="${list}" var="role">
						<tr align="center">
							<td>
								<input style="width: 18px;height: 18px;" type="radio" name="roleid" <c:if test="${role.id==userrole.coreRole.id}">checked="checked"</c:if> value="${role.id}">
							</td>
							<td>${role.name}</td>
							<td>${role.des}</td>
						</tr>
